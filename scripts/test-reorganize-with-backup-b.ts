/**
 * test-reorganize-with-backup-b.ts
 *
 * Teste controlado de reorganizeActiveSchedule pós-fix de NOT_STARTED (L1131).
 * Backup prévio em pre-teste-reorganize-28-08-b.json.
 */
...
async function main() {
  console.log("═══════════════════════════════════════════════════════════");
  console.log("  PASSO 3 (RODADA B): TESTE CONTROLADO COM L1131 FIXADO");
  console.log("═══════════════════════════════════════════════════════════\n");

  // 1. BACKUP DEDICADO COM ASSERÇÃO
  console.log("1. Executando backup 'pre-teste-reorganize-28-08-b'...");
  ...
  const backupPath = path.resolve(__dirname, "../backups/json/pre-teste-reorganize-28-08-b.json");
  ...
  console.log("═══════════════════════════════════════════════════════════");
  console.log("  MÉTRICAS DO DIFF (Passo 3.5 — Rodada B)");
  console.log("═══════════════════════════════════════════════════════════");
  ...
  for (const item of gridDepois) {
    ...
    if (b?.theoryStatus === "EXCLUDED") {
      ...
      console.log(`❌ BLOCO EXCLUDED DETECTADO: item=${item.id}, block=${b?.id}, data=${item.scheduledDate}, title=${b?.title}`);
    }
    ...
    if (b && bpPages) {
      ...
      if (!bpPages.has(key)) {
        ...
        console.log(`❌ FORA DO BLUEPRINT: item=${item.id}, sub=${subName}, pages=[${b.pageStart}–${b.pageEnd}], title=${b.title}`);
      }
    }
  }
