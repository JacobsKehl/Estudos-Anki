...
import { Play, SkipForward, Calendar, Loader2, AlertTriangle } from "lucide-react";
...
interface Props {
  ...
  isMutating: boolean;
  saveDraft: (userId: string, sessionId: string, data: { availableMinutes: number; targetQuestionCount: number }) => void;
  loadDraft: (userId: string, sessionId: string) => { availableMinutes: number; targetQuestionCount: number } | null;
  clearDraft: (userId: string, sessionId: string) => void;
}
...
export function WeeklyReviewPendingSession({
  ...
  useEffect(() => {
    const draft = loadDraft(session.userId, session.id);
    if (draft) {
  ...
  const handleMinutesChange = (newMins: number) => {
    ...
    saveDraft(session.userId, session.id, { availableMinutes: newMins, targetQuestionCount: suggested });
  };

  const handleQuestionsChange = (newQuests: number) => {
    setQuestions(newQuests);
    saveDraft(session.userId, session.id, { availableMinutes: minutes, targetQuestionCount: newQuests });
  };

  const handleStartSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    clearDraft(session.userId, session.id);
