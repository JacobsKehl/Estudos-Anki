...
const mainNavItems = [
  { label: "Estudo de Hoje", href: "/today", icon: Target },
...
const contentNavItems = [
  ...
  { label: "Biblioteca de PDFs", href: "/materials", icon: BookOpen },
];

const reviewNavItems = [
  { label: "Flashcards", href: "/flashcards", icon: BrainCircuit },
  { label: "Revisões", href: "/reviews", icon: RotateCw },
];
    ...
    <div className="space-y-1 px-2 pb-6">
      ...
      {items.map((item) => (
        <Link
          ...
          href={item.href}
          className={cn(
            "group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all hover:bg-accent/10 hover:text-accent",
            pathname === item.href
  ...
  return (
    <aside className="fixed left-0 top-0 hidden h-full w-64 flex-col border-r border-border bg-card/50 backdrop-blur-md md:flex">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-4 scrollbar-hide">
        {renderNavGroup("Principal", mainNavItems)}
        {renderNavGroup("Estudo", contentNavItems)}
        {renderNavGroup("Memorização", reviewNavItems)}
      </div>
      ...
      <div className="mt-auto border-t border-border p-4">
        <Link
          href="/settings"
          className={cn(
            "group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all hover:bg-muted/80",
            pathname === "/settings"
