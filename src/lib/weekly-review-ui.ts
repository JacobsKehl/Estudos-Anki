...
const SELECTION_REASONS: SelectionReason[] = ["WEEK_CONTENT", "OVERDUE", "LONG_UNSEEN"];

export function isSelectionReason(value: unknown): value is SelectionReason {
  return typeof value === "string" && SELECTION_REASONS.includes(value as SelectionReason);
}

/**
 * Mapeia o motivo de seleção para uma label amigável em português.
 */
export function mapSelectionReason(reason: unknown): string {
  if (isSelectionReason(reason)) {
  ...
  return typeof reason === "string" && reason ? reason : "Conteúdo da semana";
}
...
export function getDraftKey(userId: string, sessionId: string): string {
  return `${DRAFT_KEY_PREFIX}${userId}:${sessionId}`;
}

export function saveDraft(
  userId: string,
  sessionId: string,
  data: { availableMinutes: number; targetQuestionCount: number }
): void {
  try {
    sessionStorage.setItem(getDraftKey(userId, sessionId), JSON.stringify(data));
  } catch {
...
export function loadDraft(
  userId: string,
  sessionId: string
): { availableMinutes: number; targetQuestionCount: number } | null {
  try {
    const raw = sessionStorage.getItem(getDraftKey(userId, sessionId));
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object") return null;
    if (typeof parsed.availableMinutes !== "number" || typeof parsed.targetQuestionCount !== "number") return null;
    if (parsed.availableMinutes < 15 || parsed.availableMinutes > 120) return null;
    if (parsed.targetQuestionCount < 5 || parsed.targetQuestionCount > 50) return null;
    return parsed;
  } catch {
    return null;
  }
...
export function clearDraft(userId: string, sessionId: string): void {
  try {
    sessionStorage.removeItem(getDraftKey(userId, sessionId));
  } catch {
    // sessionStorage unavailable
  }
}
